...
translateAllCommand.SetHandler(async (string sourcePath, string targetPath, string targetLang) =>
{
    ...
    // Sliding window rate limiter: max 33,000 chars/minute (Azure limit)
    var rateLimitWindow = TimeSpan.FromMinutes(1);
    var maxCharsPerMinute = 33000;
    var recentTranslations = new Queue<(DateTime timestamp, int charCount)>();
    ...
    foreach (var sourceFile in markdownFiles)
    {
        try
        {
            ...
            {
                skipped++;
                Console.WriteLine($"⏭️  {relativePath} (already exists)");
                continue;
            }
            ...
            Console.Write($"📝 {relativePath} ({charCount} chars)... ");

            // Rate limit check: remove old entries outside window
            var now = DateTime.UtcNow;
            while (recentTranslations.Count > 0 && (now - recentTranslations.Peek().timestamp) > rateLimitWindow)
            {
                recentTranslations.Dequeue();
            }

            // Calculate chars in current window
            var charsInWindow = recentTranslations.Sum(x => x.charCount);

            // Wait if we would exceed rate limit
            if (charsInWindow + charCount > maxCharsPerMinute)
            {
                var oldestInWindow = recentTranslations.Peek().timestamp;
                var waitTime = rateLimitWindow - (now - oldestInWindow) + TimeSpan.FromSeconds(5); // +5s safety margin

                Console.WriteLine($"\n⏸️  Rate limit: waiting {(int)waitTime.TotalSeconds}s (window: {charsInWindow:N0}/{maxCharsPerMinute:N0} chars)");
            ...
            // Track translation for rate limiting
            recentTranslations.Enqueue((DateTime.UtcNow, charCount));
